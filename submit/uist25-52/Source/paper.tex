...
{\vspace{\dimexpr0.5\baselineskip\relax} \noindent\setlength\fboxsep{6pt}\colorbox{gray!20}{%
\parbox{\dimexpr\columnwidth-\marginparsep\relax}
{\small {#1}\; \textbf{#2.} #3}
}}
...
\item \emph{Collaborative Editing.} Users should be able to locally modify a shared document
  and merge their changes in a way that does not require a live connection to a central server~\cite{kleppmann-2019-local}.
\item \emph{Programming by Demonstration.} Allow users to construct programs by
  enacting the steps of the desired behavior using concrete examples and generalizing from those~\cite{leiva-2021-rapido,cypher-1993-pbd}.
...
\item \emph{End-User Debugging.} The user should be able to ask about provenance~\cite{cheney-2009-provenance} to understand
  why a computation resulted in a particular result and what inputs contributed to it \cite{ko-2009-whyline}.
...
\item We analyse key design decisions, alternatives and limitations (\S\ref{sec:discuss}), showing
  that the desired functionality requires a careful choice among interconnected
  design options.
\item To evaluate how Denicek simplifies the development of program\-ming systems, we
  build an innovative data science notebook and assess its implementation
...
\subsubsection*{\raisebox{-0.2em}{\includegraphics[height=1em,clip,trim=0em 0em 30mm 0em]{fig/circled.pdf}} Adding a Speaker}
...
\subsubsection*{\raisebox{-0.2em}{\includegraphics[height=1em,clip,trim=5mm 0em 25mm 0em]{fig/circled.pdf}} Creating a User Interface} To simplify adding of
...
\subsubsection*{\raisebox{-0.2em}{\includegraphics[height=1em,clip,trim=10mm 0em 20mm 0em]{fig/circled.pdf}} Abstracting the Interaction} After adding the speaker,
...
\subsubsection*{\raisebox{-0.2em}{\includegraphics[height=1em,clip,trim=15mm 0em 15mm 0em]{fig/circled.pdf}} Refactoring Document Structure} Alfred starts with the initial
...
\subsubsection*{\raisebox{-0.2em}{\includegraphics[height=1em,clip,trim=20mm 0em 10mm 0em]{fig/circled.pdf}} Merging Edits}
...
\subsubsection*{\raisebox{-0.2em}{\includegraphics[height=1em,clip,trim=25mm 0em 5mm 0em]{fig/circled.pdf}} Adding Budget Calculation} Juliana joins in and adds a budget calculation to
...
\subsubsection*{\raisebox{-0.2em}{\includegraphics[height=1em,clip,trim=30mm 0em 0mm 0em]{fig/circled.pdf}} Merging Formulas} When Juliana merges the budget calculation (F) with
...
\item \emph{Transform Matching References.} If $e_2$ targets a node that is inside a node
  ...
  corresponds to the new structure. This is done using the rules in Fig.~\ref{fig:updates},
  that apply when transforming references inside a document, although we now also support the case
  when $e1$ is \ident{Copy} (see \S\ref{app:merge-transform-refs}).
\end{enumerate}
...
\begin{tabular}{|p{25.5em}|}
\hline
\\[-1em]
\tttablecol{RenameField}{target, old, new}{\texttt{/target/old\_field/nested}}{\texttt{/target/new\_field/nested}}
  {Replace Field for matching references.}
\tttablecol{WrapRecord}{target, tag, field}{\texttt{/target/nested}}{\texttt{/target/field/nested}}
  {Insert Field selector after matching prefix.}
...
\eftablecol{ValueEffect}{Transforms value, modifies list or adds a field.}
...
\begin{figure}[t]
\includegraphics[width=1\columnwidth,clip,trim=0cm 9.5cm 8cm 0cm]{fig/coevolution.pdf}
...
\item \emph{Structure Editing.} Code in code cells should be edited through structure editor
  ...
  structure and can make the system accessible to non-programmers \cite{mcnutt-2023-projectional}.
...
\item \emph{Code Completion.} During editing, code completion should offer available data
  ...
  when using iterative prompting in The~Gamma \cite{petricek-2022-thegamma}.
...
\item \emph{Wrangling Code Synthesis.} Interactively constructed data transformations should be
  convertible into code that can be checked and further edited as in Wrex \cite{drossos-2020-wrex}.
\end{itemize}
...
\begin{acks}
The authors thank Clemens Nylandsted Klokmose for research guidance,
...
\begin{figure}[t]
\newcommand{\extablecolx}[6]{\hline\\[-0.8em]
\small{\bfseries #1} #2\\[-0.1em] \small #3\\[0.3em]
\quad \parbox{7cm}{\footnotesize #4}\\
\quad \parbox{7cm}{\vspace{0.5em}\footnotesize #6: \emph{#5}} \\[1em]
}
\newcommand{\extablecol}[5]{\extablecolx{#1}{#2}{#3}{#4}{#5}{Programming experiences}}
\begin{tabular}{|l|}
\extablecol{Counter App}{\cite{kiss-2014-7guis}}
  ...
  Formula modification is shown once and then repeated on click.
...
\extablecol{Todo App}{\cite{osmani-2024-todomvc}}
  ...
  {Functionality to add an item is shown once and then recorded.
...
\extablecol{Conference List}{\cite{edwards-2025-schema}}
  ...
  {Adding speakers to a list through an in-document user interface merges with
   refactoring that turns the list into a table and separates name from an email by
   copying the value and removing a part of the string before/after a comma.}
...
\extablecolx{Traffic Accidents}{\cite{edwards-2022-copypaste}}
  {Compute statistics using two data sources.}
  {Formula to compute statistics can be reused with a different data source. If an error
   in the formula is corrected, the correction is propagated automatically to the copied
   version of the formula.}
  {Concrete Programming, Incremental Recomputation}{Experiences}
\hline
\end{tabular}
