...
Denicek aims to support systems that feature end-user programming \cite{myers-2006-eup}, notational
freedom and self-sustainability~\cite{jakubovic-2023-techdims}, liveness, interactivity \cite{rein-2019-live}
and richness \cite{horowitz-2023-qualities}. We see programming more as interacting with a medium
or a substrate \cite{kay-1977-media,klokmose-2015-webstrates} than writing text \cite{rpg-2012-revolution}.
We first review related systems, substrates and their design (\S\ref{sec:background-sys})
before discussing specific programming experiences (\S\ref{sec:background-exp}).

We focus on systems that allow gradual progression from a user to a developer, as envisioned
in Smalltalk~\cite{reenskaug-1981-byte}. We use the term end-user programming loosely to refer to a
...
\subsubsection*{Programming Systems}
...
\subsubsection*{Computational Substrate}
explain computational substrate
Substrate defined by \cite{jakubovic-2022-ladder}
\cite{disessa-1995-epistemology}
\cite{disessa-1986-boxer}

dynamic media / webstrates
\cite{klokmose-2015-webstrates,radle-2017-codestrates}

\subsubsection*{Edit histories}
edit calculus (based on tree navigation though)
\cite{omar-2017-hazelnut}

\cite{storm-2013-deltas} semantic deltas for DSLs
Pijul and \cite{schurmann-2022-merging} merging based on lattice theory

\subsubsection*{Merging}
OTs
\cite{davis-2002-otsgml,jungnickel-2016-otjson}
more recent CRDTs
\cite{kleppmann-2019-local,litt-2022-peritext,klokmose-2024-mywebstrates}
mrdts
\cite{kaki-2019-mrdts}


Merging - c.f. mywebstrates - we need more than Automerge (cite)
which has a bunch of pre-built structures

adding move to json \cite{da-2024-jsoncrdt}

\subsection{Programming Experiences}
\label{sec:background-exp}

structure editing see \cite{beckman-2023-sandblocks}
projectional see \cite{gobert-2023-lorgnette}
...
webstrates
\cite{klokmose-2015-webstrates,klokmose-2024-mywebstrates}

\cite{wang-2024-nbconflicts}
Don't Step on My Toes: Resolving Editing Conflicts in Real-Time Collaboration in Computational Notebooks
avoid conflicts in nb editing

\cite{goldman-2011-collaborative}
Collabode: Real-time collaborative coding in a web IDE - string edit based but apply only good edits

\subsubsection*{Programming by Demonstration}
Wrangler
\cite{kandel-2011-wrangler}

\cite{myers-2000-demonstrational}
...
WREX - nicest tool (by example, but...)
\cite{drossos-2020-wrex}
generate code from edits

\cite{litt-2020-customization,litt-2020-wildcard}
Wildcard / data driven customiozation / end user customization by manipulating tabular data

\subsubsection*{Schema Evolution}
\cite{litt-2020-cambria,edwards-2025-schema}

\subsubsection*{End-User Debugging}
provenance/debugging
\cite{cheney-2009-provenance}
...
\cite{shrestha-2021-unravel}
Unravel - understand data science pipelines

\subsubsection*{Incremental Recomputation}
live/incremental recomputation
\cite{mcdirmid-2013-usable,horowitz-2023-engraft}.
\cite{petricek-2020-live}
\cite{teitelbaum-1981-cps}
\cite{acar-2006-adaptive}

in notebooks - wrattler etc

\subsubsection*{Concrete Programming}
self \cite{ungar-1987-self}
\cite{edwards-2006-copypaste,edwards-2022-copypaste}

\subsubsection*{Visualization Preview etc}
...
babylonian - live examples
\cite{rauch-2019-babylonian}

\subsubsection*{Other things}
\cite{mcnutt-2023-nbai}
AI assistants in notebook interfaces
(also notebooks, how to design AI suport)
...
* augmenters
  ...
  \note{data detectors https://dl.acm.org/doi/10.1145/272287.272331}
  \note{also see Potluck https://www.inkandswitch.com/potluck/}
