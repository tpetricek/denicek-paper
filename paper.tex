...
We use the second phase to evaluate suitability of the Denicek substrate for the development of
...
also tracked in spreadsheets \cite{hermans-2015-copypaste}. Gridlets \cite{joharizadeh-2020-gridlets}
...
picking in git). In the \emph{divergence} model, users could keep their own schema but
import all data edits. Supporting the model requires a \emph{retract} operation \cite{edwards-2021-typed}
that is dual to our edit reconcilliation (given subsequent edits $e_1, e_2$, we want
...
\ident{Add} edits (when setting the evaluation result to a computed value). For those edits, we
record additional dependencies identifying the source of the operation arguments. Those additional
dependencies also need to be considered in conflict detection.
...
a code cell (1), edits it in a grid that infers edit operations via programming by demonstration
(2), turned to a code cell (3) and then visualized.}
\label{fig:datnicek}
\vspace{-0.5em}
\end{figure*}
...
Denicek is a low-level computational substrate. It serves best as the basis for interactive
...
Datnicek, a simple notebook system that shows the ability of Denicek to support rich interactive user
...
\item \emph{Structure Editing.} Code in code cells should be edited through structure editor
  ...
  We do not yet aim to implement advanced keyboard-based editing \cite{moon-2022-tylr,beckman-2023-sandblocks}.
...
\item \emph{Interactive Data Cleaning.} It should be possible to edit data tables in an interactive
  grid and use programming by demonstration for common data cleaning tasks as in
...
\item \emph{Wrangling Code Synthesis.} Interactively constructed data transformations should be
  turned into code that can be checked by the data scientist and further edited as in Wrex \cite{drossos-2020-wrex}.
\end{itemize}
...
and other cached information. All interactions that affect the notebook
use an event that appends edits to the list of Denicek edits. Remaining events
update the transient user interface state. The support for collaborative editing has been
implemented through an ad-hoc mechanism.

\subsubsection*{User Interface}
Datnicek notebooks consists of a series of cells. In the code cells, the user can
...
``foo/bar'' using ``/'' as the delimiter adds a new column ``bar'', copies values from
...
in \S\ref{sec:system-defs}, Denicek does not allow specifying conditional selectors.
To support conditional transformations, we add a special kind of (virtual) edit
...
The development of Datnicek only started after the design and implementation of Denicek and
Webnicek was completed. Thanks to this two-phase methodology, the experience reported here
provides a qualitative evaluation of the Denicek substrate. It also highlights limitations and
suggests directions for future work.

\subsubsection*{Effectiveness and Unifromity.}
Many of the Datnicek requirements are addressed by Denicek with minimal implementation
effort. The implementation represents all non-transient actions on a notebook as Denicek edits.
...
The uniform and dynamic nature makes the development experience using Denicek more akin to dynamic
...
representation that avoids nesting, as in Histogram \cite{petricek-2019-histogram}, may be more flexible.

Variables are represented as references and automatic reference updating %(\S\ref{sec:system-defs})
prevents a number of errors. References are automatically updated when the field name changes.
Denicek also prevents the deletion of cells that contain variables to which there are references.
...
and use conflict detection to remove invalidated edits. We also uses the same logic for edits
performed in interactive grid cells, suggesting that the approach is a useful general
implementation pattern.


Because invalidation of evaluated edits is not built-in into Denicek, it is worth exploring
if Denicek could support the standard evaluation model of Jupyter notebooks based on mutable
state.

The development of Datnicek uncovered some limitations of the underlying conflict detection
in Denicek. In particular, edits generated by evaluation (such as \ident{Add} edits that set the
evaluated result) need to be treated as conflicting with edits that have both value and
structure effects. This is because they are mere results of a more complex evaluation process.

Data loaded in Datnicek is represented as Denicek document nodes (a table is a list of records).
Such uniform representation makes Denicek more general, but places a high demand on the system
performance. We implemented a number of Denicek optimisations, but Datnicek is still cumbersome
to use with data consisting of multiple thousands of rows. Improving system performance is
thus important future work.
...
akin to that of The Gamma live previews \cite{petricek-2020-foundations}.

Finally, the representation of interactive grid transformations (Fig.~\ref{fig:source})
highlights a duplication -- a transformation is represented as a sequence of edits,
but also as a formula. An interesting future direction is unifying or integrating those two
kinds of computation in the Denicek substrate.
...
\subsection{Olsen}
Situations, Tasks, Users + Importance

Generality - webnicek, denicek, map space via TDPSs
Flexibility - dynamic nature via edits - experienced in Datnicek
Expressive Match - possibly - for document based things
Can it Scale up - remains a question

\subsection{Technical Dimensions}
TDPS determine a space of designs
how much of this space can be covered by systems based on Denicek?
...
Surface and Internal Notation
Uniformity of Notations

CONCEPTUAL STRUCTRUE
not much to say
Conceptual integrity vs openness - integrity
Composability Convenience

CUSTOMIZABILITY
self-sustainability - future work

COMPLEXITY
Factoring of Complexity - no reuse for now, remains to be explored


\section{Conclusion}

knowledge exists only in the brain of system designers!!
