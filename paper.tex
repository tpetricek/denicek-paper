...
In this section, we use Webnicek, our simple web-based prototype, to illustrate the programming
experiences enabled by the Denicek substrate (\S\ref{sec:impl}). Webnicek is based on a structure
editor that supports navigation in the document and issuing of edit commands. We show two
formative examples (\S\ref{app:examples}) related to conference organization.
...
document (reference nodes) and absolute references are also used in edits (to denote a target~node).
...
indices for lists. The index of a new item has to be supplied explicitly. The design choice
simplifies merging and is discussed in \S\ref{sec:discuss}.
...
to multiple nodes simultaneously. Most edits can only be applied to target node(s) of a particular
kind.
...
point to the target of the copying after the edit (a reference cannot refer to two structurally
...
\ektablecol{\faCode}{UpdateTag}{tag}{Tagged}
...
identify common shared part of the history during conflict checking and merging.
...
provided that the reference behavior is not set to disable the updating (which is required when
the target reference contains the \ident{Index} selector). Although \ident{WrapList} has to specify
the index to be used for the newly created list item, the references in the document are updated to
use the \ident{All} selector. We assume wrapping introduces a homogeneous list and references
pointing to individual items should thus be generalized to point to all list items.
...
contains a reference that would be invalidated by the \ident{Copy} or \ident{DeleteField} edit,
...
\hline
\\[-1em]
\extablecol{Counter}{\cite{kiss-2014-7guis}}
...
\extablecol{Todo}{\cite{osmani-2024-todomvc}}
  {App with controls to add an item and remove all completed.}
